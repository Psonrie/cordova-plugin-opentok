...
class TBPublisher
  constructor: (one, two) ->
    ...
    if @properties?
      ...
      styleVideoScale = @properties.styleVideoScale ? styleVideoScale
    ...
    Cordova.exec(TBSuccess, TBError, OTPlugin, "initPublisher", [name, @position.top, @position.left, width, height, zIndex, publishAudio, publishVideo, cameraName, ratios.widthRatio, ratios.heightRatio, audioFallbackEnabled, audioBitrate, audioSource, videoSource, frameRate, resolution, styleVideoScale] )
